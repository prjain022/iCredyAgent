...
exec aca-py start \
    --inbound-transport http "0.0.0.0" ${ICREDY_ACAPY_HTTP_PORT} \
    --outbound-transport http \
    --endpoint ${ACAPY_ENDPOINT} \
    --wallet-name "${ICREDY_ACAPY_WALLET_NAME}" \
    --wallet-key "${ICREDY_ACAPY_WALLET_ENCRYPTION_KEY}" \
    --wallet-storage-config "{\"url\":\"${POSTGRESQL_HOST}:5432\",\"max_connections\":5, \"wallet_scheme\":\"${ICREDY_ACAPY_WALLET_SCHEME}\"}" \
    ...
    --admin "0.0.0.0" ${ICREDY_ACAPY_ADMIN_PORT} \
    --plugin multitenant_provider.v1_0 \
    --plugin icredy_plugins.icredy_innkeeper.v1_0 \
