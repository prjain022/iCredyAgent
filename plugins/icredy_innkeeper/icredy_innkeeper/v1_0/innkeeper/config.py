...
class InnkeeperWalletConfig(BaseModel):
    ...
    def default(cls):
        ...
        return cls(
            ...
            wallet_name="icredy_innkeeper_v1_0",
            print_key=False,
            print_token=False,
...
class iCredyInnkeeperConfig(BaseModel):
...
def get_config(settings: Mapping[str, Any]) -> iCredyInnkeeperConfig:
    """Retrieve configuration from settings."""
    try:
        LOGGER.debug("Constructing config from: %s", settings.get("plugin_config"))
        plugin_config_dict = settings["plugin_config"].get("icredy_innkeeper", {})
        ...
        default_config = iCredyInnkeeperConfig.default().model_dump()
        ...
        config = iCredyInnkeeperConfig(**config_dict)
    except KeyError:
        LOGGER.warning("Using default configuration")
        config = iCredyInnkeeperConfig.default()
